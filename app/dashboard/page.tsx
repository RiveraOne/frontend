        ...
        <section className="mw-shell">
          ...
          <div className="mw-page-header">
            <div>
              ...
              <h1 className="mw-title">Hi, {firstName}.</h1>
            ...
            <div className="flex gap-2">
              ...
              <Link href="/advisor" className="mw-btn-ghost">
                Ask AI
              </Link>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            {/* Balance */}
            <article
              className={`mw-stat-card border-l-4 ${remaining >= 0 ? "border-l-mw-accent" : "border-l-rose-400"}`}
            >
              <div className="flex items-center justify-between">
                <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-mw-soft text-mw-primary">
                  <WalletIcon />
                </div>
                <span className="text-xs font-semibold text-mw-light">
                  Balance
                </span>
              </div>
              <p
                className={`mt-3 text-3xl font-black tracking-tight ${remaining >= 0 ? "text-mw-primary" : "text-rose-600"}`}
              >
                {txLoading ? (
                  <span className="inline-block h-9 w-28 animate-pulse rounded-lg bg-mw-border" />
                ) : (
                  <>
                    ...
                    {remaining < 0 && (
                      <span className="ml-1 text-base">deficit</span>
                    )}
                  </>
                )}
              </p>
          ...
          <div className="mt-4 mw-card p-5">
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`text-sm font-bold ${spendRatio > 80 ? "text-rose-600" : "text-teal-600"}`}
              >
            ...
            <div className="h-2.5 w-full overflow-hidden rounded-full bg-mw-border">
              <div
                ...
                style={{
                  width: txLoading ? "0%" : `${Math.min(spendRatio, 100)}%`,
                }}
              />
            </div>
          ...
          <div className="mt-6">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Link
                href="/ledger"
                className="text-xs font-semibold text-mw-primary hover:underline"
              >
                View all →
              </Link>
            </div>
            <div className="mw-card divide-y divide-mw-border overflow-hidden">
              ...
              {!txError &&
                ...
                recentTransactions.map((t) => (
                  <div
                    key={t.id}
                    className="flex items-center justify-between px-5 py-3.5 hover:bg-mw-soft transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                    ...
                    <span
                      className={`text-sm font-bold ${t.type === "Income" ? "text-teal-600 dark:text-teal-400" : "text-rose-600 dark:text-rose-400"}`}
                    >
                      {t.type === "Income" ? "+" : "-"}$
              ...
              {!txError && !txLoading && recentTransactions.length === 0 && (
                <div className="px-5 py-8 text-center">
                  <p className="text-sm font-semibold text-mw-primary">
                    No transactions yet
                  </p>
                  <p className="mt-1 text-xs text-mw-body">
                    Add your first transaction to build a snapshot.
                  </p>
                </div>
              )}
            </div>
